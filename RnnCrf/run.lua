...
dofile 'build_rnn.lua'
dofile 'evaluate_func.lua'
dofile 'train_model.lua'
dofile 'experiment.lua'
dofile 'util_function.lua'

local config = {dim_in=300, activation='sigmoid', num_tag=3, maxIter=20, bSize=32, K=3}
config.window_size = 3
config.optim_config={learningRate=0.01}
config.optim_config.weightDecay=1e-5*config.bSize
local direc = '/home/yingding/SequenceLabeling/RNN/data_large_vocab/'
local para = {}
...
take = arg[4]
config.dim_hidden = config.dim_out

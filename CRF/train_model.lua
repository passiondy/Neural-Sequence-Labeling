...
function train_model.train(data, config)
    ...
    for iter=1, maxIter do
        ...
        for b = 1,bNum do
            ...
            local feval = function()
                ...
                for i=(b-1)*bSize+1, math.min(b*bSize, N) do
                    local idx = idxList[i]
                    local loss = model:forward(seq_train[idx], tag_train[idx])
                    model:backward(seq_train[idx], tag_train[idx])
                    bLoss = bLoss + loss
        ...
        local perf
        local perf_test
        perf = train_model.cal_performance(model, {tag=data.tag_train, seq=data.seq_train})
        print('Train f-score:', perf[3])
        print(perf[1], perf[2])
        print(perf[6], perf[7])
        if valid then
